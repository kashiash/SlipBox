...
Gdy jest zaznaczone, możemy również dodać do niego atrybuty, takie jak tytuł mojej notatki, a następnie musimy zdefiniować typ. Jeśli nie zdefiniujemy typu, otrzymamy błąd. Tu idę za opisem błędu, który mówi: "Tytuł notatki nie może mieć niezdefiniowanego typu atrybutu. Musi mieć zdefiniowany typ". Jeśli zmienimy to na jeden z domyślnych, np. tytuł to rodzaj ciągu znaków, błąd znika. 
...
Możemy zmienić to na coś innego, jak `Manual/None`, ale wtedy musisz wszystko napisać sam. Nie rób tego. Jest dość skomplikowane.  



Możemy zobaczyć, jak generuje to plik, jak tylko pozbędziemy się problemów zgłaszanych przez Xcode. Zaczniemy od rozwiązania jednego problemu. Problem dotyczy pliku "PersistentFile", dodajemy tu pozycje Item, ktorych już nie mamy wiec zamieniamy je na Notę. 

 `**let** newNote = Note(context: viewContext)`

 Zobaczysz, jak tworzymy obiekt `Note` i jak Core Data wymaga posiadania kontekstu, który mówi, że ten obiekt należy do tej bazy danych.  Mogę ustawić wszystkie swoje atrybuty, takie jak tytuł, które będą pobierane z "title". Przykład ustawiania daty utworzenia na bieżącą. 
...
W pliku "Persistent" możemy teraz korzystać z tego wygodnego inicjalizatora. Możemy korzystać z właściwości "title" i "notes", na przykład w kontekście indeksu, jeśli chcemy zobaczyć różne daty utworzenia. Możemy również zmienić datę utworzenia, szczególnie w trybie podglądu.
...
W AddNote i deleteNote Item mamy powtarzajacy sie kod zapisujacy dane, przeniemy go do naszego rozszerzenia klasy:
